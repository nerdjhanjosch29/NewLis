<?php
require_once '../Database/connection.php';
$data = json_decode(file_get_contents('php://input'));
  $UserID = "";
  $validateToken = include('../validate_token.php');
  $UserID = $decode->UserID;
  if(!$validateToken)
  {
    http_response_code(404);
    die();
  }
if(isset($data))
{
        $TruckingName = 0;
        $TruckingID = 0;
        $ContactNumber = "";
        $ContactPerson = "";
        $Status = 0;
        if($data->TruckingID)
        {
        $TruckingID = $data->TruckingID;
        }
        if($data->TruckingName)
        {
        $TruckingName = $data->TruckingName;
        }
        if($data->ContactPerson)
        {
        $ContactPerson = $data->ContactPerson;
        }
        if($data->ContactNumber)
        {
        $ContactNumber = $data->ContactNumber;
        }
            $sql = "EXEC [dbo].[Truckings]
                    @TruckingID = ?,
                    @TruckingName = ?,
                    @ContactPerson = ?,
                    @ContactNumber = ?,
                    @Status = ?,
                    @UserID = ?";
            $params = array($TruckingID,$TruckingName,$ContactPerson,$ContactNumber,$Status,$UserID);
            $stmt = sqlsrv_query($conn, $sql, $params);
            $result = 0;
            while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC))
            {
                echo $result = $row['result'];
            }

             if($result != 0 )
            {       
                $SystemLogID = 0;
                $FunctionID = 1; 
                $TableName = "Trucking";
                $Activity = "Insert Trucking : $TruckingName";
                $UpdatedData = "0";
                $sql1 = "EXEC	[dbo].[SystemLogs]
                          @SystemLogID = ?,
                          @UserID = ?,
                          @FunctionID = ?,
                          @TableName = ?,
                          @Activity = ?,
                          @UpdatedData = ?";
                  $paramss = array($SystemLogID,$UserID,$FunctionID,$TableName,$Activity,$UpdatedData);       
                  $stmt = sqlsrv_query($conn, $sql1, $paramss);
            }
          
              sqlsrv_commit($conn);  
}
?>